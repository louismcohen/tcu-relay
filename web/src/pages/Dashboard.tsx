...
export function Dashboard({ onLoggedOut }: DashboardProps) {
  ...
  const [stream, setStream] = useState<"sse" | "poll">("sse");
    ...
    <main className="mx-auto min-h-screen max-w-6xl px-6 py-8">
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4 border-b border-line pb-6">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-medium">LiveWire telemetry</h1>
        </div>
        <div className="flex items-center gap-3">
          <Badge className={stream === "sse" ? "border-phosphor text-phosphor" : ""}>
            {stream}
          </Badge>
          <Button
            variant="ghost"
      ...
      {snapshot === undefined ? (
        <p className="font-mono text-mute">Waiting for first snapshot…</p>
      ) : (
...
function CtechColumn({ snapshot }: { readonly snapshot: StatusSnapshot }) {
  ...
  return (
    <Card>
      ...
      <Readout label="WS" value={ctech.wsState} accent={ctech.wsState === "connected"} />
      ...
      <Readout
        label="GPS"
        value={
          ctech.latitude !== undefined && ctech.longitude !== undefined
            ? `${ctech.latitude.toFixed(5)}, ${ctech.longitude.toFixed(5)}`
            : undefined
        }
      />
      <Readout label="Last msg" value={formatAge(ctech.lastMessageAt)} />
      <Readout label="Token exp" value={ctech.tokenExpiry} />
      <Readout label="HD update" value={ctech.hdLastUpdate} />
      <Readout label="GPS ts" value={ctech.gpsTimestamp} />
  ...
  return (
    <Card>
      ...
      <Readout
        ...
        accent={abrp.lastResult === "ok" || abrp.lastResult === "dry_run"}
      />
      <Readout label="Last send" value={formatAge(abrp.lastSentAt)} />
      <Readout label="Missing" value={abrp.lastMissing} warn />
      <Readout
        label="Backoff"
        value={abrp.backoffMs !== undefined ? `${String(abrp.backoffMs)} ms` : undefined}
      />
      <div className="mt-4 border-t border-line pt-4">
        <p className="mb-2 text-[11px] uppercase tracking-[0.16em] text-mute">
          Last tlm
        </p>
        <pre className="max-h-72 overflow-auto font-mono text-[11px] leading-5 text-phosphor">
...
function RelayColumn({ snapshot }: { readonly snapshot: StatusSnapshot }) {
  return (
    <Card>
      ...
      <Readout label="Vehicle" value={snapshot.vehicleId} />
      <Readout label="Dry run" value={snapshot.dryRun ? "yes" : "no"} />
      <Readout label="Interval" value={`${String(snapshot.sendIntervalMs)} ms`} />
      <Readout label="Uptime" value={`${String(snapshot.uptimeSeconds)} s`} />
      <Readout label="Started" value={snapshot.startedAt} />
    </Card>
  );
}
